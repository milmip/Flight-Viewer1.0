...
for fileN in list_file_name:
    ...
    with open(f"hd/{fileN}", 'r') as inn:
        with open(f"trans/{fileN}", 'w') as edited:
            for line in inn:
                edited.write(line)

    with open(f"trans/{fileN}", 'a+') as edited:
        with open(f"hd/{req1_name}", 'r') as file1:
            y_search = tile[1]*1000-21
            #print(y_search)
            for line in file1:
        ...
        with open(f"hd/{req2_name}", 'r') as file2:
            x_search = (tile[0]+1)*1000+21
            #print(x_search)
            for line in file2:
        ...
        with open(f"hd/{req3_name}", 'r') as file3:
            for _ in range(52):
    ...
    with open(f"trans/{fileN}", 'r') as file:
        with open(f"hd3/{fileN}", 'w') as edited2:
            for line in file:
                if int(line[0:7]) == x_search-20:
                    edited2.write(line)
                    try:
